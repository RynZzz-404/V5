...
│
└─「 ɪɴꜰᴏ ʙᴏᴛ 」       
│ᴏᴡᴇɴʀ ᴛᴀɢ :  @${ownernya.split('@')[0]}
│ᴅɪᴅᴜᴋᴜɴɢ  : @${ini_mark.split('@')[0]}
│ᴘʀᴇғɪx :   ${prefix}
...
│ʏᴏᴜʀ ɴᴀᴍᴇ : ${pushname}
│ʙɪᴏ : ${bio}
│ɴᴏ.ᴛᴀɢ : @${m.sender.split('@')[0]}
│ʙᴏᴛ : ${m.key.fromMe ? 'True' : 'False'}
│ᴍʏ ᴏᴡɴᴇʀ : ${isCreator ? 'True' : `False`}
│
└┬────────────┈ ❖
   │❖  ʙᴇʀɪ ᴊᴇᴅᴀ 3 ᴅᴇᴛɪᴋ 
   │❖  ᴊᴀɴɢᴀɴ sᴘᴀᴍ ʙᴏᴛ
...
const buttojns = [
  ...
  {buttonId: 'donasi', buttonText: {displayText: 'ᴅᴏɴᴀᴛᴇ'}, type: 1}
]			
					...
					if(typemenu == 'templateLocation'){
						await alpha.send5ButLoc(from, lang.menunya(salam, pushname, botname) , `© ${ownername}`,pp_bot, [{"urlButton": {"displayText": "ᴍʏ ᴡᴇʙ","url": `${youtube}`}},{"urlButton": {"displayText": "ɢʀᴏᴜᴘ ᴏғᴄ","url": `${myweb}`}},{"quickReplyButton": {"displayText": "ᴅᴏɴᴀᴛᴇ","id": 'donate'}},{"quickReplyButton": {"displayText": "ᴏᴡɴᴇʀ","id": 'owner'}},{"quickReplyButton": {"displayText": "ʟɪsᴛ ᴄᴏᴍᴍᴀɴᴅ","id": 'command'}}], { userJid: m.chat, quoted: m } )
							}
						if(typemenu == 'templateTenor'){
							alpha.send5ButGif(from, lang.menunya(salam, pushname, botname) , `© ${ownername}` ,pp_bot, [{"urlButton": {"displayText": "ᴍʏ ᴡᴇʙ","url": `${youtube}`}},{"urlButton": {"displayText": "ɢʀᴏᴜᴘ ᴏғᴄ","url": `${myweb}`}},{"quickReplyButton": {"displayText": "Donasi","id": 'donate'}},{"quickReplyButton": {"displayText": "Owner","id": 'owner'}},{"quickReplyButton": {"displayText": "List Command","id": 'command'}}] , {quoted: m})
						 }
						if(typemenu == 'katalog'){
					...
					break    
	case 'allmenu':{
			await alpha.send5ButLoc(from, `Hai kak ${pushname} 👋, saya *${botname}* ` + '\n\n' + lang.listMenu(time, salam, pushname, prefix) , `© ${ownername}`,pp_bot, [{"urlButton": {"displayText": "ᴍʏ ᴡᴇʙ","url": `${youtube}`}},{"urlButton": {"displayText": "ɢʀᴏᴜᴘ ᴏғᴄ","url": `${myweb}`}},{"quickReplyButton": {"displayText": "ᴅᴏɴᴀsɪ","id": 'donate'}},{"quickReplyButton": {"displayText": "ᴏᴡɴᴇʀ","id": 'owner'}},{"quickReplyButton": {"displayText": "ʀᴜʟᴇs","id": 'rules'}}] )
		}
	break   
	case 'infocmd': case'infomenu':{
		alpha.sendButImage(m.chat, sender, [{buttonId: '.command',buttonText:{displayText: 'ᴋᴇᴍʙᴀʟɪ'},type: 1},{buttonId:'owner',buttonText: {displayText:'ᴏᴡɴᴇʀ'}, type: 1}], `Selamat ${salam} @${sender.split('@')[0]}\n\n╭─⏣「 _*INFO BOT*_ 」⏣\n│ *Prefix* :  ${prefix} \n│ *Name* : ${botname}\n│ *Owner* : @${ownernomer.split("@")[0]}\n│ *Mode* : ${alpha.public ? 'Public-Mode' : 'Self-Mode'}\n│ *Runtime* : ${runtime(process.uptime())}\n│ *Lib* : Baileys-Md@4.0.0\n╰─⏣` + '\n\n' +  lang.info(prefix), `© ${ownername}`, [sender, ownernomer + '@s.whatsapp.net'], {quoted:m})
      }
  break 
